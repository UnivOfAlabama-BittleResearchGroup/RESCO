...
class GAProblem(ElementwiseProblem):
    def __init__(self, env_args: argparse.Namespace, *args, **kwargs):
        ...
        super().__init__(
            n_var=agent.ga_nvars, n_obj=1, n_ieq_constr=0, xl=0, xu=5, **kwargs
        )
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        env.close(save_metrics=False)
        env.save_metrics(
            [
                (
                    "total_reward.txt",
                    lambda f: f.write(str(out["F"])),
                ),
                # save the X
                (
                    "ga_params.txt",
                    lambda f: np.savetxt(f, x),
                ),
            ]
        )
...
def ga_optimizer(
    ...
    res: Result = minimize(
        ...
        GA(
            sampling=LHS(),
            mutation=PolynomialMutation(prob=0.4),
            # callback=MyCallback(),
            verbose=True,
        ),
